...
int melodyStar[] = {
 ...
 int tempoStar = 108;
int sizeStar = sizeof(melodyStar) / sizeof(melodyStar[0]);
int noteStar = (60000 * 4) / tempoStar; 
...
int tempoMario = 200;
int sizeMario = sizeof(melodyMario) / sizeof(melodyMario[0]);
int noteMario = (60000 * 4) / tempoMario; 


void mario(){
    int noteLen = abs(noteMario / melodyMario[currNoteIndex + 1]);

    if (melodyMario[currNoteIndex + 1] < 0) {
      noteLen *= 1.5; 
    }

    if (millis() - musicTiming > noteLen){
      currNoteIndex += 2;
      if (currNoteIndex >= sizeMario){
        currNoteIndex = 0;
      }
      ledcWriteTone(AUDIO_PWM, melodyMario[currNoteIndex]);
      musicTiming = millis();
    } else if (currNoteIndex == 0){
      ledcWriteTone(AUDIO_PWM, melodyMario[currNoteIndex]);
    }
...
int tempoHarry = 144;
int sizeHarry = sizeof(melodyHarry) / sizeof(melodyHarry[0]);
int noteHarry = (60000 * 4) / tempoHarry; // this calculates the duration of a whole note in ms (60s/tempo)*4 beatsS

//BEFORE ANY SONG TRANSITIONS
//musicTiming = millis();
//currNoteIndex = 0;



void harryPotterLoop(){

    int noteLen = abs(noteHarry / melodyHarry[currNoteIndex + 1]);

    if (melodyHarry[currNoteIndex + 1] < 0) {
      noteLen *= 1.5; 
    }

    if (millis() - musicTiming > noteLen){
      currNoteIndex += 2;
      if (currNoteIndex >= sizeHarry){
        currNoteIndex = 0;
      }
      ledcWriteTone(AUDIO_PWM, melodyHarry[currNoteIndex]);
      musicTiming = millis();
    } else if (currNoteIndex == 0){
      ledcWriteTone(AUDIO_PWM, melodyHarry[currNoteIndex]);
    }
...
int tempoPink = 120;
int sizePink = sizeof(melodyPink) / sizeof(melodyPink[0]);
int notePink = (60000 * 4) / tempoPink; 

void pinkPanther(){

  int noteLen = abs(notePink / melodyPink[currNoteIndex + 1]);

    if (melodyPink[currNoteIndex + 1] < 0) {
      noteLen *= 1.5; 
    }

    if (millis() - musicTiming > noteLen){
      currNoteIndex += 2;
      if (currNoteIndex >= sizePink){
        currNoteIndex = 0;
      }
      ledcWriteTone(AUDIO_PWM, melodyPink[currNoteIndex]);
      musicTiming = millis();
    } else if (currNoteIndex == 0){
      ledcWriteTone(AUDIO_PWM, melodyPink[currNoteIndex]);
    }
...
int tempoThrone = 85;
int sizeThrone = sizeof(melodyThrone) / sizeof(melodyThrone[0]);
int noteThrone = (60000 * 4) / tempoThrone; 

void throne(){
       int noteLen = abs(noteThrone / melodyThrone[currNoteIndex + 1]);

    if (melodyThrone[currNoteIndex + 1] < 0) {
      noteLen *= 1.5; 
    }

    if (millis() - musicTiming > noteLen){
      currNoteIndex += 2;
      if (currNoteIndex >= sizeThrone){
        currNoteIndex = 0;
      }
      ledcWriteTone(AUDIO_PWM, melodyThrone[currNoteIndex]);
      musicTiming = millis();
    } else if (currNoteIndex == 0){
      ledcWriteTone(AUDIO_PWM, melodyThrone[currNoteIndex]);
    }
